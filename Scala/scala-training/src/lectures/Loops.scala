...
object Loops extends App {
  ...
  val h = for (a <- 1 to 20) yield {
    s"$a is the number"
  }

  println(h)

  val i = for (a <- 1 to 20) yield a * 2


  val numbers = 1 to 10

  val doubles = for (a <- numbers) yield {
    a * 2
  }

  println(numbers)
  println(doubles)  // original range or collection is not modified. a new collection is yielded
